...
export const Route = createFileRoute("/pulse-studio")({
  head: () => ({
    meta: [
      { title: "MedTrail Pulse Studio | Super Admin" },
      {
        name: "description",
        content: "MedTrail Pulse Studio — Official daily pulses and live operations control center.",
      },
      { name: "robots", content: "noindex, nofollow" },
    ],
  }),
  component: PulseStudioPage,
});

function PulseStudioPage() {
          ...
          <Reveal className="p-8 sm:p-10 rounded-3xl bg-slate-950/90 border border-rose-500/40 text-center shadow-2xl space-y-6 backdrop-blur-xl relative overflow-hidden">
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs sm:text-sm text-slate-400 leading-relaxed max-w-md mx-auto">
                Pulse Studio is strictly restricted to authorized MedTrail Administrators.
              </p>
            </div>
            ...
            {user ? (
              <div className="p-4 rounded-2xl bg-slate-900/80 border border-slate-800 text-xs text-left space-y-2 font-mono">
                ...
                <div className="text-[11px] text-slate-500 font-sans">
                  This email is not authorized for Admin clearance.
                </div>
              </div>
            ) : (
              <div className="p-4 rounded-2xl bg-slate-900/80 border border-slate-800 text-xs text-center text-slate-400">
                You are currently not signed in. An authenticated Admin session is required.
              </div>
            )}
